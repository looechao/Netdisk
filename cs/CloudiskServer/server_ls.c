...
// 
//     // 准备 SQL 查询语句
//     const char *sql = "SELECT file_name FROM file_table WHERE parent_id = ? AND tomb = 'y'";
...
//     if (!meta_result) {
//         write_log(mysql_stmt_error(stmt), "error", NULL);
//         mysql_stmt_close(stmt);
//         return;
//     }
// 
...
//         write_log("Failed to bind result", "error", NULL);
...
//         write_log("Failed to store result", "error", NULL);
//         mysql_free_result(meta_result);
//         mysql_stmt_close(stmt);
//         return;
//     }
